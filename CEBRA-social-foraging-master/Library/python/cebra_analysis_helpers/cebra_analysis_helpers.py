...
def run_single_grid(output_dir,mouse_and_date_list, neural_data_list, conditions_dict_list, indices_of_trials_each_frame_list,
    ...
    """
    formatted = f"{lr:.0e}".replace("e-0", "e-").replace("e+0", "e+")
    save_info_str = f"{m_str}_bs{batch_size}_tmp{temperature}_Miter{max_iter}_h{num_hidden_units}_lr{formatted}"
    dir_path = f"{output_dir}demo_{save_info_str}/"
    behavior_approach_path = f"{dir_path}{behavior}_{approach}_{tmux_session_name}/"
    ...
    model_dump_file = f"{behavior_approach_path}multi_cebra_model_{tmux_session_name}.pkl"
    # Save embeddings in current folder
    with open(model_dump_file, 'wb') as f:
        pickle.dump(multi_cebra_model, f)
    ax = cebra.plot_loss(multi_cebra_model)
    loss_save = f"{behavior_approach_path}LOSS_{tmux_session_name}.png"
    ...
    dump_file = f"{behavior_approach_path}train_valid_dict_{tmux_session_name}.pkl"
