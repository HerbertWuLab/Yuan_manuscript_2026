...
def plot_gridworld_trajectories(grid_H, grid_W, traj, fig, ax):
    '''
    plot trajectory in gridworld.  use color gradient to indicate time
    '''

    ax.imshow(np.zeros((grid_H,grid_W)), alpha=0)
    ...
    # points = np.array([y, x]).transpose().reshape(-1, 1, 2)
    ...
    cbar = fig.colorbar(lines, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_ticks([0, 1])
    cbar.set_ticklabels(['Start', 'End'])



def plot_gridworld_trajectories_noindex(grid_H, grid_W, traj, color, ax, noise_level = 0.1):
    '''
    without the grid index
    ...
    states2d = traj['states2d']
    np.random.seed(0)
    x = states2d[:,1] + np.random.normal(scale=noise_level, size=len(states2d[:,1])) # jitter traj slightly
    np.random.seed(1)
    y = states2d[:,0] + np.random.normal(scale=noise_level, size=len(states2d[:,0])) # jitter traj slightly
    # x = states2d[:,1]
    # y = states2d[:,0]
    t = np.linspace(0,1,len(x))
    ...
    # make the collection of segments
    lc = LineCollection(segs, colors=[color], linewidths=2)
    lc.set_array(t)  # color the segments by our parameter
    lines = ax.add_collection(lc);  # add the collection to the plot
    # cbar = fig.colorbar(lines, ax=ax, fraction=0.046, pad=0.04)
    # cbar.set_ticks([0, 1])
