...
from src.helpers import extract_state_action_pair, create_joint_maps_pytorch, create_joint_maps_pytorch_v2, create_joint_maps_HD_pytorch
import torch
import time
...
def getMAP_goalmaps(seed, P_a, trajectories, hyperparams, a, individual_map1, individual_map2, diff_map, 
    ...
    if info['inter_tag'] == 6: # self map + interaction map
        if info['SingleAgent'] == 1:
            individual_map2 = torch.zeros_like(individual_map2)
            individual_map2.requires_grad = False
        elif info['SingleAgent'] == 2:
            individual_map1 = torch.zeros_like(individual_map1)            
    ...
    if info['inter_tag'] == 8: # two individual maps
        diff_map = torch.zeros_like(diff_map)
        diff_map.requires_grad = False
    if info['inter_tag'] == 9: # only interaction map
        individual_map1.requires_grad = False
        individual_map2.requires_grad = False
...
def getLL(individual_map1, individual_map2, diff_map, state_action_pairs, hyperparams, a, P_a, gamma, info, tag, width, height):
    ...
    if info['HD']:
        joint_map = create_joint_maps_HD_pytorch(individual_map1, individual_map2, diff_map, info=info, width=width, height=height)
        # rewards = (a.T @ joint_map.reshape(a.shape[0],-1)).reshape((1,joint_map.shape[1],joint_map.shape[2])) # 1 x N_states x N_angles
        A_reshaped = (a.T).view(1, 3, 1, 1) # (1 x 3 x 1 x 1)
        B_reshaped = joint_map.unsqueeze(0)     # (1 x 3 x N_STATES**2 x N_ANGLES)
    ...
    if info['HD']:
        for i in range(num_trajectories):
            states = torch.tensor(state_action_pairs[i][:,0], dtype=torch.long)
            actions = torch.tensor(state_action_pairs[i][:,1], dtype=torch.long)
            angles = torch.tensor(state_action_pairs[i][:,2], dtype=torch.long)
